...
get_maf_filtered_genotype <- function(genotype_file_name,  maf, samples) {
  gt_df <- read.table(genotype_file_name, header = T, stringsAsFactors = F, row.names = 1, sep = "\t")
  updated_colnames_gt_df <- sub(pattern= "X", replacement= "", x = colnames(gt_df))
  colnames(gt_df) <- updated_colnames_gt_df
...
get_gene_expression <- function(gene_expression_file_name, gene_annot) {
  expr_df <- as.data.frame(t(read.table(gene_expression_file_name, header = T, stringsAsFactors = F, row.names = 1, sep = "\t")))
...
get_covariates <- function(covariate_file_name, samples) {
  cov_df <- read.table(covariate_file_name, header = TRUE, stringsAsFactors = FALSE)
  updated_colnames_cov_df <- sub(pattern= "X", replacement= "", x = colnames(cov_df))
  colnames(cov_df) <- updated_colnames_cov_df
...
##changed values of MAF from 0.01 to 0.05

main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
                 covariates_file, chrom, prefix, maf=0.05, n_folds=10, n_train_test_folds=5,
